    ...
    <span class="pull-right header-action">
      <a-dropdown style="height:64px;display:inline-block;">
        <a class="ant-dropdown-link">
          <a-avatar src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png" style="position:relative;" />
  ...
  methods: {
    ...
    changeLang() {
      if (this.language.label === '简体中文') {
        this.$store.commit('sys/setLanguage', {label: 'English', value: zh_CN})
        i18n.locale = 'cn'
      } else {
        this.$store.commit('sys/setLanguage', {label: '简体中文', value: en_US})
        i18n.locale = 'en'
      }
    },
    handleClick() {
